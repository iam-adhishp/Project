...
# garm_list = os.listdir(os.path.join(example_path,"static/assets/cloth"))
# garm_list_path = [os.path.join(example_path,"static/assets/cloth",garm) for garm in garm_list]

# human_list = os.listdir(os.path.join(example_path,"static/assets/human"))
# human_list_path = [os.path.join(example_path,"static/assets/human",human) for human in human_list]
...
#         #     examples=[
#         #         [{% static "assets/examples/model2.png" %}, {% static "assets/examples/garment2.png" %}, {% static "assets/examples/result2.png" %}],
#         #         [{% static "assets/examples/model3.png" %}, {% static "assets/examples/garment3.png" %}, {% static "assets/examples/result3.png" %}],
#         #         [{% static "assets/examples/model1.png" %}, {% static "assets/examples/garment1.png" %}, {% static "assets/examples/result1.png" %}],
#         #     ],
...
def tryon_view(request):
    if request.method == "POST":
        ...
        randomize_seed = request.POST.get("randomize_seed")
        
        if not person_img or not garment_img:
            return JsonResponse({"status": "error", "message": "Empty image"})
        
        if randomize_seed == "true":
            seed = random.randint(0, MAX_SEED)
        
        # Convert images to base64
        person_img_np = np.frombuffer(person_img.read(), np.uint8)
        person_img_cv = cv2.imdecode(person_img_np, cv2.IMREAD_COLOR)
        encoded_person_img = base64.b64encode(cv2.imencode(".jpg", person_img_cv)[1].tobytes()).decode("utf-8")
        
        garment_img_np = np.frombuffer(garment_img.read(), np.uint8)
        garment_img_cv = cv2.imdecode(garment_img_np, cv2.IMREAD_COLOR)
        encoded_garment_img = base64.b64encode(cv2.imencode(".jpg", garment_img_cv)[1].tobytes()).decode("utf-8")
        
        url = f"http://{os.environ.get('tryon_url', '')}Submit"
        headers = {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if status == "success":
                    ...
                    return JsonResponse({"status": "error", "message": "Processing failed"})
            else:
                return JsonResponse({"status": "error", "message": "Try-on service unavailable"})
        except Exception as e:
            return JsonResponse({"status": "error", "message": str(e)})
        
        time.sleep(9)  # Wait before fetching the result
        ...
        for _ in range(max_retries):
            try:
                result_url = f"http://{os.environ.get('tryon_url', '')}Query?taskId={uuid}"
                response = requests.get(result_url, headers=headers, timeout=20)
                ...
                if response.status_code == 200:
                    ...
                    if status == "success":
                        ...
                        return JsonResponse({"status": "success", "image": base64.b64encode(buffer.tobytes()).decode("utf-8")})
                    elif status == "error":
        ...
        return JsonResponse({"status": "error", "message": "Too many retries, try again later"})
    
    return render(request, "try/tryon.html")
